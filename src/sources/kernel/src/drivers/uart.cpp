...
bool CUART::Open()
{
    ...
    sGPIO.Set_GPIO_Function(14, NGPIO_Function::Alt_0);
    sGPIO.Set_GPIO_Function(15, NGPIO_Function::Alt_0);
...
char* CUART::Read(char* pc) 
{
    while (!(mAUX.Get_Register(hal::AUX_Reg::MU_LSR) & 1))
       ;

    *pc = (mAUX.Get_Register(hal::AUX_Reg::MU_IO) & 0xFF);
    return pc;
}

int CUART::Read(char* buf, int size)
{
    char ch;
    int i = 0;
    Read(&ch);
