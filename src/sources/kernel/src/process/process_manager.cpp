...
void CProcess_Manager::Handle_Process_SWI(NSWI_Process_Service svc_idx, uint32_t r0, uint32_t r1, uint32_t r2, TSWI_Result& target)
{
    ...
    switch (svc_idx)
    {
        ...
        case NSWI_Process_Service::RandomNumberRequest:
        if(sTRNG.Open(true))
        {
            target.r0 = sTRNG.Get_Random_Number();
        }
        sTRNG.Close();
        break;
        case NSWI_Process_Service::FirstPageRequest:
        pt = sPT_Alloc.Alloc();
        ActualPage = static_cast<unsigned long>(sPage_Manager.Alloc_Page()) - mem::MemoryVirtualBase;
        map_memory(pt, ActualPage, r1);
        target.r0 = r1;
        target.r1 = *pt;
        break; 
        case NSWI_Process_Service::PageRequest:
        ActualPage = static_cast<unsigned long>(sPage_Manager.Alloc_Page()) - mem::MemoryVirtualBase;
        map_memory(&r2, ActualPage, r1);
        target.r0 = r1;
        break;   
    }
}
